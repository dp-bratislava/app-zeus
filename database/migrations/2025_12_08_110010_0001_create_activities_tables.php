...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        Schema::create($tablePrefix . 'activity_template_groups', function (Blueprint $table) use ($tablePrefix) {
            $table->comment('Hierarchical groups of activities');
            $table->id();
            $table->string('code')
                ->nullable()
                ->comment('Code can be unique for department or for group, but not overall unique');
            $table->foreignId('parent_id')
                ->nullable()
                ->comment('Used for hierarchical structure of groups.')
