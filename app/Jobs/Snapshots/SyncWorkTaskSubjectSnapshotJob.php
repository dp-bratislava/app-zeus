...
class SyncWorkTaskSubjectSnapshotJob implements ShouldQueue
{
    ...
    public function handle(): void
    {
        ...
        //     ->where('maintainable_id', '=', 3)
        //     // ->whereNotNull('maintainable_id')
        //     // ->orWhere('deleted_at', '>', $lastSync)
        //     ->orderBy('id')
        //     ->pluck('id');

        $ids = DB::table('dpb_worktimefund_model_task')
            ->join('dpb_worktimefund_mm_morphable_attributeoption', 'dpb_worktimefund_mm_morphable_attributeoption.morphable_id', '=', 'dpb_worktimefund_model_task.id')
            ->select('dpb_worktimefund_model_task.id')
            ->distinct()
            ->orderBy('dpb_worktimefund_model_task.id')
            ->pluck('id');
